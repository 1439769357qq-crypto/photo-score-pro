...
@Slf4j
@RestController
@RequestMapping("/api/photo-score")
...
public class PhotoScoreController {
    ...
    public Result<String> scoreBatchAsync(@RequestParam("files") List<MultipartFile> files,
        ...
        if (files.size() > 20) {
            return Result.error("上传照片的数量超过限制，一次只能上传最多20张照片");
        }
        String taskId = UUID.randomUUID().toString();
        String clientIp = getClientIp(request);
        String userAgent = request.getHeader("User-Agent");

        // 关键：必须在当前请求线程里缓存文件内容，不能把 MultipartFile 原对象传进异步线程
        List<CachedMultipartFile> cachedFiles = new ArrayList<>();
        ...
        try {
            for (MultipartFile file : files) {
                ...
                cachedFiles.add(new CachedMultipartFile(file));
            }
        } catch (Exception e) {
            log.error("缓存上传文件失败", e);
