...
public abstract class BaseScorer {
    ...
    protected double calculateRawScore(ScoringImageContext context) {
        return calculateRawScore(context.getImage());
    }

    protected String generateComment(double rawScore, ScoringImageContext context) {
        return generateComment(rawScore, context.getImage());
    }
    ...
    protected List<String> generateSuggestions(double rawScore, ScoringImageContext context) {
        return generateSuggestions(rawScore, context.getImage());
    }
    ...
    public ScoringResult score(ScoringImageContext context) {
        long startTime = System.currentTimeMillis();
        double rawScore = calculateRawScore(context);
        BigDecimal finalScore = BigDecimal.valueOf(rawScore * 100)
                .setScale(2, RoundingMode.HALF_UP);
        String comment = generateComment(rawScore, context);
        List<String> suggestions = generateSuggestions(rawScore, context);
        long elapsed = System.currentTimeMillis() - startTime;
        log.debug("评分器 [{}] 完成评分: raw={}, score={}, 耗时={}ms",
                getScorerName(), String.format("%.4f", rawScore), finalScore, elapsed);
        return new ScoringResult(getScorerName(), getCategory(), finalScore,
