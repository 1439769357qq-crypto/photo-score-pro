...
public class CompositionScorer extends BaseScorer {
    ...
    protected double calculateRawScore(BufferedImage image) {
        try (ScoringImageContext context = ScoringImageContext.from(image)) {
            return calculateRawScore(context);
        }
    }

    @Override
    protected double calculateRawScore(ScoringImageContext context) {
        Mat mat = context.getBgr();
        Mat gray = context.getGray();
        int width = mat.cols();
        int height = mat.rows();

        Point visualCenter = calculateVisualCenter(gray);
        double thirdScore = evaluateRuleOfThirds(visualCenter, width, height);
        double simplicityScore = evaluateSimplicity(context);
     ...
     */
    private Point calculateVisualCenter(Mat gray) {
        Mat sobelX = null;
        Mat sobelY = null;
        Mat magnitude = null;
        try {
            ...
            safeRelease(sobelX, sobelY, magnitude);
        }
    }
    ...
    private double evaluateSimplicity(ScoringImageContext context) {
        Mat edges = context.getEdges50To150();
        int edgePixels = Core.countNonZero(edges);
        double edgeDensity = (double) edgePixels / (edges.cols() * edges.rows());
    ...
    private double evaluateSymmetry(Mat mat) {
        ...
        Mat diff = null;
        try {
            leftHalf = new Mat(mat, new Rect(0, 0, halfWidth, height));
            rightHalf = new Mat(mat, new Rect(width - halfWidth, 0, halfWidth, height));
            ...
            safeRelease(leftHalf, rightHalf, rightFlipped, diff);
        }
    }
